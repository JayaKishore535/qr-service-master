...
public enum DKG_QR_Mode {
  ...
  DKG_QR_Mode(int[] characterCountBitsForVersions, int bits) {
    this.characterCountBitsForVersions = characterCountBitsForVersions;
    this.bits = bits;
  }


  public static DKG_QR_Mode forBits(int bits) {
    switch (bits) {
      case 0x0:
        return TERMINATOR;
      case 0x1:
  ...
  public int getCharacterCountInBits(DKG_Version DKGVersion) {
    int number = DKGVersion.getVersionNumber();
    int offset;
    if (number <= 9) {
      offset = 0;
    } else if (number <= 26) {
